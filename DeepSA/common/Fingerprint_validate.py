...
def test(test_set, predictor_proseq):
    test_score = predictor_proseq.evaluate(test_set, detailed_report=False)
    return test_score
...
def validate_model(key, value):
    ...
    np.random.seed(1207)
    predictor_proseq = TabularPredictor.load(path=model_path)
    data_set = pd.read_csv(val_set, dtype={'id':str, label:str, 'smiles':str})
    data_set['fingerprint'] = data_set['smiles'].apply(lambda x:gen_fingerprint(x, fingerprint_type="ECFP4"))
    data_set = data_set[['fingerprint',label]]
    val_data = pd.DataFrame(data_set['label']).join(pd.DataFrame(data_set['fingerprint'].to_list()), how='left')
